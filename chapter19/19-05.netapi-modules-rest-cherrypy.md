...
3. 可选的配置: 生成自签的 SSL 证书。强烈建议使用安全的HTTPS连接，因为Salt eauth身份验证凭据将通过网络发送。
   - 安装 pyOpenSSL 软件包。
   - 使用执行模块函数 [create_self_signed_cert()](https://docs.saltstack.com/en/latest/ref/modules/all/salt.modules.tls.html#salt.modules.tls.create_self_signed_cert) 生成一个自签的数据证书。
```bash
